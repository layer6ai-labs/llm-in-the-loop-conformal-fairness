...
from internal.dataset.datasets import acs_downloader
...
class ACSIncome(CustomDataset):
    ...
    def get_data(
        ...
        **kwargs,
        ...
        # If CSVs are missing in data_root, attempt to download/process ACS data
        required_files = [
            "income_features.csv",
            "income_labels.csv",
            "income_sens.csv",
        ...
        files_exist = all(
            os.path.exists(os.path.join(data_root, f)) for f in required_files
        )
        if not files_exist:
            try:
                print("ACS income CSVs not found — running acs_downloader.download_acs()")
                acs_downloader.download_acs()
            except Exception as e:
                print(f"Warning: failed to auto-download ACS data: {e}")

        feature_path = os.path.join(data_root, "income_features.csv")
        features = pd.read_csv(feature_path)
        label_path = os.path.join(data_root, "income_labels.csv")
        labels = pd.read_csv(label_path)
        labels = labels.rename(columns={"income_bracket": "label"})
        group_path = os.path.join(data_root, "income_sens.csv")
    ...
    # TabPFN
    # def get_model1(self, device, train_loader, val_loader, **kwargs):
    #     model = TabPFN()
    ...
    def get_model(self, device, train_loader, val_loader, **kwargs):
        ...
        if model_checkpoint is not None:
            if model_checkpoint is True:
                model_checkpoint = "logs/model_checkpoints/AcsIncome.ubj"
        ...
        # grid = GridSearchCV(
        #     xgb_base, param_grid, cv=3, scoring="roc_auc_ovr", n_jobs=5, verbose=3
        # )
    ...
    def process_dataframe(self, df, loader_dict, k):
        ...
        df = bring_examples_to_top(df, 10, min_num)
