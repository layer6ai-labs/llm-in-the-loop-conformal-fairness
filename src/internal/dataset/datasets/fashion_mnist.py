...
from internal.model.networks import ConvNet
...
class Fashion_MNIST(CustomDataset):
    ...
    def get_data(
        ...
        **kwargs,
        ...
        val_dset = GroupDataset(
            "val", self.transform(train_data.data[:n_val]), val_targets, val_targets
        )
        train_targets = train_data.targets[n_val:]
        train_dset = GroupDataset(
            "train",
            self.transform(train_data.data[n_val:]),
            train_targets,
            train_targets,
        )

        calib_targets = test_data.targets[:n_calib]
        calib_dset = GroupDataset(
            "calib",
            self.transform(test_data.data[:n_calib]),
            calib_targets,
            calib_targets,
        )
        ...
        test_dset = GroupDataset(
            "test", self.transform(test_x), test_targets, test_targets
        )
        ...
        **kwargs,
        ...
        if model_checkpoint is not None:
            if model_checkpoint is True:
                model_checkpoint = "logs/model_checkpoints/FashionMNIST.pt"
    ...
    def process_dataframe(self, df, loader_dict, k):
        ...
        df["conformal_marginal_text"] = df.apply(
            lambda x: prediction_set_text_fn(x["conformal_marginal_set"], id2label_fm),
            axis=1,
        )
        df["conformal_conditional_text"] = df.apply(
            lambda x: prediction_set_text_fn(
                x["conformal_conditional_set"], id2label_fm
            ),
            axis=1,
        )
        df["conformal_backward_text"] = df.apply(
            lambda x: prediction_set_text_fn(x["conformal_backward_set"], id2label_fm),
            axis=1,
        )
        df["conformal_clustered_label_text"] = df.apply(
            lambda x: prediction_set_text_fn(x["conformal_clustered_label_set"], id2label_fm),
            axis=1,
        )

        df["conformal_clustered_group_text"] = df.apply(
            lambda x: prediction_set_text_fn(x["conformal_clustered_group_set"], id2label_fm),
            axis=1,
        )
