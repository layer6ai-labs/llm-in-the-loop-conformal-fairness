...
from internal.model.networks import BiosBiasModel
from substantive.faircp.conformity.utils import ConformalCategory


def ensure_nltk_resource(resource: str, path_prefix: str = "tokenizers/"):
    try:
        find(f"{path_prefix}{resource}")
    except LookupError:
...
class BiosBias(CustomDataset):
    ...
    def get_data(
        ...
        **kwargs,
        ...
        calib_subset_df, d_conf_remain = stratified_sample_df(
            d_conf_df, col="label", n_samples=n_calib // m, return_remaining=True
        )
        print(f"d_conf_remain: {len(d_conf_remain)}")
        test_subset_df, d_conf_remain = stratified_sample_df(
            d_conf_remain, col="label", n_samples=n_test // m, return_remaining=True
        )
        print(f"d_conf_remain: {len(d_conf_remain)}")
        calib_val_subset_df, _ = stratified_sample_df(
            d_conf_remain,
            col="label",
            n_samples=n_calib_val // m,
        ...
        train_subset = Dataset.from_pandas(
            stratified_sample_df(
                dataset_dict["train"], col="label", n_samples=n_train // m
            )
        )

        val_subset = Dataset.from_pandas(
            stratified_sample_df(
    ...
    def filter_bios(self, datasets, m=10):
        ...
        for split in datasets:
            datasets[split] = list(
                filter(lambda x: x["label"] not in [1, 2, 13], datasets[split])
            )
    ...
    def get_model(self, device, train_loader, val_loader, **kwargs):
        ...
        if model_checkpoint is not None:
            if model_checkpoint is True:
                model_checkpoint = "logs/model_checkpoints/BiosBias.pt"
        ...
        for epoch in range(kwargs["epochs"]):
            ...
            for idx, data in enumerate(train_loader):
                optimizer.zero_grad()
                inputs = (
                    torch.stack(data["avg_enc"], dim=1).to(torch.float32).to(device)
                )
    ...
    def process_dataframe(self, df, loader_dict, k):
        ...
        df["conformal_backward_text"] = df.apply(
            lambda x: prediction_set_text_fn(x["conformal_backward_set"], id2label_ge),
            axis=1,
        )
        df["conformal_clustered_label_text"] = df.apply(
            lambda x: prediction_set_text_fn(x["conformal_clustered_label_set"], id2label_ge),
            axis=1,
        )

        df["conformal_clustered_group_text"] = df.apply(
            lambda x: prediction_set_text_fn(x["conformal_clustered_group_set"], id2label_ge),
            axis=1,
        )
