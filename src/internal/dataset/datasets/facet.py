...
from internal.model.networks import FACETModel
...
class FACET(CustomDataset):
    ...
    def get_data(
        ...
        **kwargs,
        ...
        calib_subset, calib_val_subset, test_subset, classnames = (
            self.create_and_split_facet(data_root, n_calib, m)
        )
    ...
    def create_and_split_facet(self, vis_root, n_calib, m=20):
        ...
        inner_dataset = torchvision.datasets.ImageFolder(
            os.path.join(vis_root, "images")
        )
        annotations_df = pd.read_csv(
            os.path.join(vis_root, "annotations/annotations.csv")
        )

        # Only keep images with single person
        nb_person_per_image = (
            annotations_df.groupby(["filename"])["person_id"].count().reset_index()
        )
        nb_person_per_image = nb_person_per_image.rename(
            columns={"person_id": "nb_person"}
        )
        annotations_df_nb_person = annotations_df.merge(
            nb_person_per_image, left_on="filename", right_on="filename"
        )
        single_person_images = annotations_df_nb_person.loc[
            annotations_df_nb_person.nb_person == 1
        ]

        # Only keep images with single label
        filtered_annotations_df = single_person_images.loc[
            single_person_images["class2"].isnull()
        ].reset_index()

        # Concatenate groups into a single feature
        age_info = filtered_annotations_df.loc[
            :, filtered_annotations_df.columns.str.startswith("age_")
        ]
        ...
        self.group_map = {
            group: group_text for group, group_text in enumerate(groupnames)
        }
        ...
        self.label_map = {
            label: label_text for label, label_text in enumerate(classnames)
        }
        ...
        if n_calib <= 0:
            ...
            n_calib = n_calib - (
                n_calib % m
            )  # ensure that dataset will be class balanced
            print(
        ...
        for path, _ in inner_dataset.imgs:
            ...
            row = filtered_annotations_df.loc[
                filtered_annotations_df.filename == filename
            ]

            if not row.empty:
                label = row["class1"].iloc[0]  # scalar
                age_group = row["age_groups"].iloc[0]
                if label in classnames:
                    ...
                    if num_examples_per_class_calib[class_idx] < min_num_calib:
                        ...
                        num_examples_per_class_calib_val[class_idx] < min_num_calib_val
                    ):
        ...
        calib_val_dataset = self.FACET(
            vis_processor, classnames, classes, calib_val_annotation
        )
    ...
    def process_dataframe(self, df, loader_dict, k):
        ...
        df["conformal_marginal_set"] = df["conformal_marginal_set"].apply(
            relabel_set_obj, label_reordering=label_reordering
        )
